...
/// Selección de perfiles (PC) → al elegir va directo al MainHome de PC.
...
class _ProfileSelectionPageState extends State<ProfileSelectionPage> {
  ...
  void _syncFocusNodes(int n) {
    while (_profileNodes.length < n) {
      _profileNodes.add(
        FocusNode(debugLabel: 'profile_${_profileNodes.length}'),
      );
    }
    while (_profileNodes.length > n) {
  ...
  /// Al elegir perfil en PC → siempre al MainHome de PC.
  Widget _resolveHome() {
    if (widget.buildHome != null) return widget.buildHome!();
    return const PcMainHome();
  }
  ...
  Future<void> _createProfile() async {
    ...
    if (profile != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No se pudo crear el perfil. Revisa tablas SQL y conexión.',
          ),
          backgroundColor: Colors.red,
        ),
      );
  ...
  Future<void> _deleteProfile(Map<String, dynamic> profile) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title:
            const Text('Eliminar perfil', style: TextStyle(color: Colors.white)),
        content: Text(
  ...
  // ── Tamaños (pensados para PC) ─────────────────────────────────────────
  double _circleRadius(Size size) {
    final side = size.shortestSide;
    if (side < 500) return 52;
    if (side < 700) return 62;
    if (side < 1000) return 72;
    return 80;
  }
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(
                            ...
                            fontSize: size.shortestSide < 700 ? 14 : 16,
                          ),
                        ),
                        const SizedBox(height: 40),
                        Expanded(
                                ...
                                return Padding(
                                  ...
                                  child: TextButton(
                                    ...
                                    child: Text(
                                      ...
                                      style: TextStyle(
                                        fontSize: size.shortestSide < 700
                                            ? 14
                                            : 15,
                                        fontWeight: focused
                                            ? FontWeight.w600
  ...
  Widget _buildProfilesGrid(Size size) {
    final radius = _circleRadius(size);
    final canCreate = _profiles.length < 5;
    final items = <Widget>[];

    for (int i = 0; i < _profiles.length; i++) {
      items.add(_buildProfileCircle(_profiles[i], i, radius, size));
    }
    if (canCreate) {
      items.add(_buildCreateCircle(radius, size));
    }
      ...
      child: Builder(
        builder: (ctx) {
          ...
          return GestureDetector(
            ...
            onSecondaryTap: () => _deleteProfile(profile),
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    ...
                    fontSize: size.shortestSide < 700 ? 14 : 15,
  ...
  Widget _buildCreateCircle(double radius, Size size) {
    return Focus(
