...
class ExtractorHlsService {
  ...
  static Future<String?> buscarFuente(
    ...
    // 2. WebView oculto (Windows = webview_windows / WebView2)
    ...
    entry = OverlayEntry(
      builder: (_) => Positioned(
        // En Windows WebView2 necesita un tamaño mínimo real para inicializar
        left: useWindows ? 0 : -5,
        top: useWindows ? 0 : -5,
        width: useWindows ? 2 : 1,
        height: useWindows ? 2 : 1,
        child: IgnorePointer(
          child: Opacity(
            opacity: 0.01,
            child: useWindows
...
// ============================================================
//  WEBVIEW2 OCULTO (Windows — webview_windows)
// ============================================================

class _HiddenProbeWindows extends StatefulWidget {
  final String servidorUrl;
  final Duration timeout;
  final ValueChanged<String?> onResult;

  const _HiddenProbeWindows({
    required this.servidorUrl,
    required this.timeout,
    required this.onResult,
  });
  ...
  @override
  State<_HiddenProbeWindows> createState() => _HiddenProbeWindowsState();
}

class _HiddenProbeWindowsState extends State<_HiddenProbeWindows> {
  ...
  Future<void> _boot() async {
    try {
      ...
      // Mensajes desde JS → Dart
      try {
        _msgSub = _controller.webMessage.listen((msg) {
          if (_resuelto) return;
          final url = msg?.toString().trim() ?? '';
          if (url.isNotEmpty && _isMediaUrl(url)) {
            _addDetectedUrl(url);
          }
        });
      ...
      _loadSub = _controller.loadingState.listen((state) {
        if (_resuelto) return;
        if (state == LoadingState.navigationCompleted) {
          unawaited(_injectDetector());
        }
      });

      await _controller.loadUrl(widget.servidorUrl);
      if (mounted) setState(() => _ready = true);

      // Polling extra por si webMessage no llega
      _pollTimer = Timer.periodic(const Duration(seconds: 2), (_) {
        if (!_resuelto) unawaited(_pollFromJs());
      });
  ...
  void _addDetectedUrl(String url) {
    if (_resuelto) return;
    final absoluteUrl = _toAbsoluteUrl(url);
    if (!_isMediaUrl(absoluteUrl)) return;
    if (_detectadas.add(absoluteUrl)) {
      if (absoluteUrl.contains('.m3u8') || absoluteUrl.contains('.mp4')) {
        _finalizar(absoluteUrl);
      }
  ...
  Future<void> _injectDetector() async {
    try {
      // webview_windows: postMessage vía chrome.webview o window.chrome.webview
      await _controller.executeScript(r'''
        (function() {
          if (window.__mdWinInjected) return;
          window.__mdWinInjected = true;
          const urls = new Set();
          ...
          function send(url) {
            ...
            try {
              ...
              if (!(abs.includes('.m3u8') || abs.includes('.mp4') || abs.includes('.m4s'))) return;
              if (urls.has(abs)) return;
              urls.add(abs);
              try {
                if (window.chrome && window.chrome.webview && window.chrome.webview.postMessage) {
                  window.chrome.webview.postMessage(abs);
                }
              } catch(e) {}
            } catch(e) {}
          }
          ...
          try {
            ...
            XMLHttpRequest.prototype.open = function(method, url) {
              try { if (url) send(String(url)); } catch(e) {}
          ...
          function scan() {
            ...
            try {
              document.querySelectorAll('video, source, [src]').forEach(function(el) {
            ...
            try {
              if (window.jwplayer) {
                ...
                pl.forEach(function(item) {
                  ...
                  if (item.sources) item.sources.forEach(function(s) { if (s.file) send(s.file); });
                });
              }
            } catch(e) {}
          }
  ...
  Future<void> _pollFromJs() async {
    try {
      // executeScript en webview_windows a veces no devuelve valor;
      // el detector ya postea por webMessage. Aquí reforzamos scan.
      await _controller.executeScript(r'''
        (function() {
          try {
            performance.getEntriesByType('resource').forEach(function(e) {
              ...
              if (u.includes('.m3u8') || u.includes('.mp4')) {
                if (window.chrome && window.chrome.webview) {
                  window.chrome.webview.postMessage(u);
                }
              }
            });
          } catch(e) {}
