...
//
// Concurrency de verificación: SharedPreferences key
//   'servidores_concurrent_checks' (1–5, default 2)
...
class FuentesConfig {
  ...
  /// Cuántas verificaciones HLS en paralelo (Settings → RENDIMIENTO).
  final int concurrentChecks;
  ...
  static Future<FuentesConfig> load() async {
    ...
    final concurrent =
        (prefs.getInt(keyConcurrentChecks) ?? 2).clamp(1, 5);
...
// ─────────────────────────────────────────────────────────────
// Cola de verificación (N concurrentes según Settings) + caché
// ─────────────────────────────────────────────────────────────

class _VerifyGate {
  static int _inflight = 0;
  /// Límite activo (Settings). Default 2. Se actualiza con [setMaxConcurrent].
  static int _maxConcurrent = 2;
  static final List<Completer<void>> _waiters = [];
  static final Map<String, String> _memCache = {};

  /// Aplica el valor de Settings (1–5). Llamar tras loadConfig.
  static void setMaxConcurrent(int n) {
    _maxConcurrent = n.clamp(1, 5);
  }
  ...
  static void release() {
    _inflight = (_inflight - 1).clamp(0, 999);
...
class MainFuentes {
  ...
  Future<FuentesConfig> loadConfig() async {
    ...
    // Sincroniza el gate de verificación con Settings
    _VerifyGate.setMaxConcurrent(_config!.concurrentChecks);
    return _config!;
  }

  /// Permite forzar concurrency sin recargar toda la config.
  void setMaxConcurrent(int n) {
    _VerifyGate.setMaxConcurrent(n);
  }

  int get maxConcurrent => _VerifyGate.maxConcurrent;
  ...
  Stream<FuenteEvent> fetchProgressive({
    ...
    // Asegura concurrency actualizada (por si Settings cambió)
    _VerifyGate.setMaxConcurrent(cfg.concurrentChecks);
    ...
    // Fuentes en paralelo (cada una scrape + verify limitado por _VerifyGate)
        ...
        try {
          await for (final raw in _scrapeFuente(
            ...
            if (verificar &&
              ...
              verifiedAttempts++;
              // acquire/release dentro de verificarConExtractor → respeta N paralelo
